...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/Middlebury', split='F'):
        ...
        lines_all = list(map(osp.basename, glob(os.path.join(root, "midd_all/*"))))
        print(lines)
        print(lines_all)
        ...
        all_left_list = sorted([os.path.join(root, "midd_all", f'{name}/im0.png') for name in lines_all])
        all_right_list = sorted([os.path.join(root, "midd_all", f'{name}/im1.png') for name in lines_all])
        all_disp = sorted([os.path.join(root, "midd_all", f'{name}/disp0GT.pfm') for name in lines_all])
        ...
        # for img1, img2, disp in zip(all_left_list, all_right_L_list, all_disp):
        #     self.image_list += [[img1, img2]]
        #     self.disparity_list += [disp]
        # for img1, img2, disp in zip(all_left_list, all_right_E_list, all_disp):
        #     self.image_list += [[img1, img2]]
        #     self.disparity_list += [disp]

class GS2MESH_DTU(StereoDataset):
    def __init__(self, aug_params=None, root="data_for_finetune", test_or_train="test", scans=[105]):
